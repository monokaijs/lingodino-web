  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if (hasIntroImage) {
        ...
        cmd
          .input(normalizedIntroImagePath)
          .inputOptions(['-loop 1', '-framerate 30', '-f image2', `-t ${introDuration}`]);
        // Scale Intro Image + Trim
        filters.push(
          `[0:v]trim=duration=${introDuration},setpts=PTS-STARTPTS,scale=${width}:${height}:force_original_aspect_ratio=increase,crop=${width}:${height},setsar=1,fade=t=out:st=${introDuration - 0.5}:d=0.5[v]`
        );
      } else {
        ...
        filters.push(
          `[0:v]trim=duration=${introDuration},setpts=PTS-STARTPTS,drawbox=x=0:y=0:w=iw:h=ih:color=white:t=fill[whitebg]`
        );
        filters.push(
          `[whitebg]drawtext=text='${safeText}':fontcolor=black:fontsize=64:x=(w-text_w)/2:y=(h-text_h)/2:fontfile='C\\:/Windows/Fonts/arial.ttf',fade=t=out:st=${introDuration - 0.5}:d=0.5[v]`
        );
      }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      // Video Scale + Trim
      filters.push(
        `[0:v]trim=duration=${mainDuration},setpts=PTS-STARTPTS,scale=${width}:${height}:force_original_aspect_ratio=increase,crop=${width}:${height},setsar=1,fade=t=in:st=0:d=0.5[v]`
      );
    ...
    const originalSegments =
      conversation.alignment && conversation.alignment.segments ? conversation.alignment.segments : [];
