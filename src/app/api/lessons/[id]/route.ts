...
async function putHandler(request: NextRequest, context: {params: Promise<{id: string}>}) {
  ...
  const updated = await dbService.lesson.update(
    {_id: id},
    {
      ...
      conversationId: body.conversationId ?? lesson.conversationId,
      vocabularyIds: body.vocabularyIds ?? lesson.vocabularyIds,
      grammarIds: body.grammarIds ?? lesson.grammarIds,
    },
    {new: true}
  );
