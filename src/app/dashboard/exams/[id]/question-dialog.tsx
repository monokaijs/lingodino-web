    ...
    if (!data.options || data.options.length === 0) {
      ...
      data.options.forEach((opt, i) => {
        if (!opt.text || opt.text.trim() === "") {
          ctx.addIssue({code: z.ZodIssueCode.custom, message: "Option text is required", path: ["options", i, "text"]});
        }
      });
    }
  }
...
function OptionRow({index, form, onRemove, canRemove}: {index: number; form: any; onRemove: () => void; canRemove: boolean}) {
      ...
      <div className="flex-1 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
            <IconUpload className="h-3 w-3 mr-1" />{uploading ? "..." : "File"}
          </Button>
          {optionFile && (
            <div className="flex items-center gap-1 text-xs">
              <span className="truncate max-w-[150px]">{optionFile.name}</span>
              <Button type="button" variant="ghost" size="icon-sm" onClick={() => form.setValue(`options.${index}.file`, undefined)}>
...
export function QuestionDialog({open, onOpenChange, examId, question}: QuestionDialogProps) {
  ...
  useEffect(() => {
    if (question) {
      ...
      form.reset({type: ExamQuestionType.MultipleChoice, question: "", answer: "", options: [{text: "", isCorrect: false}]});
          ...
          <form onSubmit={form.handleSubmit(onSubmit, onError)} className="space-y-4">
            <FormField control={form.control} name="type" render={({field}) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent>
                    {Object.values(ExamQuestionType).map(t => <SelectItem key={t} value={t}>{questionTypeLabels[t]}</SelectItem>)}
                  </SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
            ...
            <FormField control={form.control} name="question" render={({field}) => (
              <FormItem>
                ...
                <FormControl><Textarea placeholder="Enter question" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
                  <IconUpload className="h-4 w-4 mr-1" />{uploading ? "Uploading..." : "Upload"}
                </Button>
                {questionFile && (
                  <div className="flex items-center gap-1 text-sm">
                    <span>{questionFile.name}</span>
                    <Button type="button" variant="ghost" size="icon-sm" onClick={() => setQuestionFile(null)}><IconX className="h-3 w-3" /></Button>
                  </div>
                )}
              </div>
            </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {fields.map((field, index) => (
                    <OptionRow key={field.id} index={index} form={form} onRemove={() => remove(index)} canRemove={fields.length > 1} />
                  ))}
                </div>
              </div>
            )}
              ...
              <FormField control={form.control} name="answer" render={({field}) => (
                <FormItem>
                  ...
                  {watchedType === ExamQuestionType.TrueFalse ? (
                    <FormControl>
                      <ToggleGroup type="single" value={field.value} onValueChange={field.onChange} variant="outline" className="justify-start">
                        <ToggleGroupItem value="true">True</ToggleGroupItem>
                        <ToggleGroupItem value="false">False</ToggleGroupItem>
                      </ToggleGroup>
                    </FormControl>
                  ) : (
                    <FormControl><Input placeholder="Correct answer" {...field} /></FormControl>
                  )}
                  <FormMessage />
                </FormItem>
              )} />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={mutation.isPending}>{mutation.isPending ? "Saving..." : isEditing ? "Update" : "Create"}</Button>
            </div>
          </form>
        </Form>
      </DialogContent>
