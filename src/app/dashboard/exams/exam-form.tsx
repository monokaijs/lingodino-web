...
export function ExamForm({exam}: ExamFormProps) {
  ...
  const mutation = useMutation({
    mutationFn: (data: ExamFormData) =>
      isEditing ? updateExam(exam._id, data) : createExam(data),
    onSuccess: () => {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                render={({field}) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {lessons?.map((lesson) => (
                          <SelectItem key={lesson._id} value={lesson._id}>{lesson.name}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <FormMessage />
              ...
              <FormField control={form.control} name="description" render={({field}) => (
                <FormItem>
                  ...
                  <FormControl><Textarea placeholder="Exam description" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
